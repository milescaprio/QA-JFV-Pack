...
#accepter rabbit    summon rabbit ~ ~ ~ {Age:-25000,RabbitType:4,Invulnerable:1,PersistenceRequired:0,Silent:1,NoAI:1,Tags:["accepter_flag"],ActiveEffects:[{Id:14,Amplifier:0,Duration:999999,ShowParticles:0b}]}
#/tellraw @a {"text":"","color":"blue","extra":[{"selector":"@p"},{"text":" has won the game!"}]}

#tick down trade timeout timer
scoreboard players remove @a[scores={offer_trade_timer=0..}] offer_trade_timer 1
...
execute as @a[tag=offering_trade,tag=offerer] run tellraw @s {"text":"You are offering a trade!","color":"green","extra":[{"text":"[Click Here] or use /trigger cancel to cancel!","color":"aqua","clickEvent":{"action":"run_command","value":"/trigger cancel"},"hoverEvent":{"action":"show_text","value":"/trigger cancel"}}]}
scoreboard players set @a[tag=offerer] offer_trade_timer 1200
execute as @a[tag=offering_trade,tag=offerer] run tellraw @a               {"text":"","color":"blue","extra":[{"selector":"@a[tag=offerer]"},{"text":" offers a trade! It will expire in 60 seconds!"}]}
#         [tag=!offerer]
...
execute as @a[scores={offer_trade_timer=1..}] run function jfv:count_trades
execute as @a[scores={offer_trade_timer=1..}] run function jfv:handle_trade_errors
execute as @a[scores={offer_trade_timer=0}] run tellraw @a {"text":"","color":"blue","extra":[{"selector":"@a[tag=offerer]"},{"text":"'s trade has expired!"}]}
execute as @a[scores={offer_trade_timer=0}] run function jfv end_trade

tag @a[scores={accept_trade=1..}] add accepting_trade
scoreboard players set @a[tag=accepting_trade] accept_trade 0

tag @a[scores={confirm_trade=1..}] add confirming_trade
scoreboard players set @a[tag=confirming_trade] confirm_trade 0
...
execute as @a[scores={view_trade=1..}] run execute at @e[tag=offerer_flag,limit=1] run tellraw @s {"text":"The items offered are:\n ","color":"green","extra":[{"nbt":"Items[]","storage":"offer1","color":"aqua"}]}
